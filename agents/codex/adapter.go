...
// Adapter converts between canonical Agent and OpenAI Codex CLI agent format.
type Adapter struct{}
...
// FileExtension returns the file extension for Codex agents.
func (a *Adapter) FileExtension() string {
	return ".md"
}

// DefaultDir returns the default directory name for Codex agents.
func (a *Adapter) DefaultDir() string {
	return "agents"
}

// Parse converts Codex agent Markdown bytes to canonical Agent.
func (a *Adapter) Parse(data []byte) (*core.Agent, error) {
	...
	agent := &core.Agent{
		...
		Model:        mapCodexModelToCanonical(frontmatter["model"]),
...
// Marshal converts canonical Agent to Codex agent Markdown bytes.
func (a *Adapter) Marshal(agent *core.Agent) ([]byte, error) {
	...
	if agent.Model != "" {
		buf.WriteString(fmt.Sprintf("model: %s\n", mapCanonicalModelToCodex(agent.Model)))
	}
...
// ReadFile reads a Codex agent Markdown file and returns canonical Agent.
...
// WriteFile writes canonical Agent to a Codex agent Markdown file.
